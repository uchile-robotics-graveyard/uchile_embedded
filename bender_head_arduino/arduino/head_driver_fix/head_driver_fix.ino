...
#define HEAD_MODEL 100
#define HEAD_FIRMWARE 100
#define HEAD_MMAP_SIZE 4
...
/**
 * @brief Head control using DXL communication protocol
 * @details Head control using Dynamixel communication protocol over RS485.
 * This implementation uses 4 uint8_t variables to control state of 5 servo motors and 2 NeoPixel arrays. 
 * 
 * @param dir_pin Toggle communication pin.
 * @param reset_pin Pin for reset device
 * @param servo_pin SERVO pin.
 */
class HeadDXL: public DeviceDXL<HEAD_MODEL, HEAD_FIRMWARE>
{
  public:
	HeadDXL(uint8_t dir_pin, uint8_t reset_pin, uint8_t numServos, uint8_t servos_pins[]):
    DeviceDXL(HEAD_MMAP_SIZE), // Call parent constructor
    ...
    void init(uint8_t servos_pins[], Servo servos[], Adafruit_NeoPixel LEDs[])
    {
  	...
  	//void updateServos(Servo servos[], uint8_t servo_select, uint8_t servo_pos, uint8_t command)
  	void updateServos(Servo servos[])
  	{
  		if (servo_select_command_.data == 5)	//when both values updated, change state
  		{
  ...
  private:
    ...
    // SERVOs variables    
    MMap::Variable<UInt8, UInt8::type, 0U, 6U, 0U> servo_select_command_;
    MMap::Variable<UInt8, UInt8::type, 0U, 180U, 0U> servo_pos_command_;

    // LEDs variables
    MMap::Variable<UInt8, UInt8::type, 0U, 254U, 0U> led_select_command_;
    MMap::Variable<UInt8, UInt8::type, 0U, 254U, 0U> led_color_command_;
